...
{
    public class CurrencySpawner : ASpawner
    {
        public CurrencySpawner(IMainFactory factory, SpawnersManager spawnersManager) : base(factory, spawnersManager)
        {
        }

        protected override IEntity CreateObject(Transform parent, Vector2 position, Quaternion rotation)
        {
            return _factory.CreateCurrency(parent, position, rotation);
        }
